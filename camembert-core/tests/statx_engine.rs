...
//!
//! The parity tests (and their fixture) are Linux-only, gated below at
//! `mod parity` granularity rather than for the whole file: `io_uring` is a
//! Linux kernel interface with no counterpart to be at parity *with* on
//! Windows (`StatxEngine::IoUring` always falls back to sync there — see
//! `Scanner::scan`'s `backend::probe_uring`). The remaining test,
//! `engine_selection_is_reported_and_never_fails`, needs none of the
//! fixture's Unix-only setup and is exactly the case worth keeping
//! cross-platform: forcing an engine that does not exist on this platform
//! must still resolve to *some* backend and never fail the scan.
...
fn engine_selection_is_reported_and_never_fails() {
    ...
    // Forcing io_uring must never fail either: on denial (including "this
    // platform has no io_uring at all", which is what Windows and non-Linux
    // Unixes report) it falls back.
...
/// The engine-parity tests: bucket 1 (intrinsically Linux). `io_uring` is a
/// Linux kernel interface, so "does io_uring statx agree with sync statx"
/// has no meaning on a platform that never has an io_uring backend to begin
/// with — there is no narrower cut than gating the whole comparison.
#[cfg(target_os = "linux")]
mod parity {
